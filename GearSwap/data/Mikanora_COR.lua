...
--[[ WS Sets ]]--
	...
	sets.precast.WS['Savage Blade'] = set_combine(sets.precast.WS, {		
		...
		ring2="Karieyh Ring",
		back=gear.sWSDCape,
		waist="Fotia Belt", -- Fotia?
		legs="Meghanada Chausses +2", -- Upgrade to Herculean STR/Att/Acc/WSD
		feet="Lanun Bottes +3",
	})
...
--[[ COR Sets ]]--
	...
	sets.precast.CorsairRoll.Duration = {range="Compensator", main="Rostam", neck="Regal Necklace"}
...
--[[ Utility Sets ]]--
	...
	-- TH
	sets.TreasureHunter = {head="White Rarab Cap +1", waist="Chaac Belt"} -- Upgrade to whatever TH we can find
...
function init_modetables()
	...
	weaponMode = {
        ...
        [1] = {name="Naegling-Tauret", set={main="Naegling", sub="Tauret"}, color="\\cs(0,191,255)"},  
		[2] = {name="Tauret-Gleti",set={main="Tauret", sub="Gleti's Knife"}, color="\\cs(238,130,238)"},
		[3] = {name="Naegling-Shield", set={main="Naegling", sub="Nusku Shield"}, color="\\cs(218,165,32)"}, 		
		[4] = {name="LowDMG-Dagger", set={main="Qutrub Knife", sub="Extinction"}, color="\\cs(218,165,32)"},
		[5] = {name="LowDMG-Sword", set={main="Nihility", sub="Extinction"}, color="\\cs(218,165,32)"},       
	}
	if not player.sub_job == 'NIN' and not player.sub_job == 'DNC' then
		weaponMode.index = 3
	end
...
function extendedJobPostMidcast(spell, action, spellMap, eventArgs)
	...
	elseif spell.type == 'WeaponSkill' then
		if spell.name == "Leaden Salute" or spell.name == "Wildfire" or spell.name == "Hot Shot" then
			if(dayWeatherIntensity(spell.element)) > 0 then
				equip({waist="Hachirin-no-Obi"})
			end
		end
...
function autoActions()
    ...
    if (auto.fite[auto.fite.index] == 'On' and not actionInProgress and not moving and me.status == 1) then
        ...
        if (player.tp >= 200 and curingWaltzRecast == 0 and partyLowHP(30)) then
