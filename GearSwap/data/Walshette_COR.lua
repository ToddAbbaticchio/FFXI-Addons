...
function extendedUserSetup()
	...
	send_command('bind @z gs c cyclezMode')
...
function extendedJobSelfCommand(cmdParams, eventArgs)	
	...
	if cmdParams[1] == 'cyclezMode' then
        auto.zMode.index = auto.zMode.index + 1
        if auto.zMode.index > #auto.zMode then
            auto.zMode.index = 0
        end
        windower.add_to_chat(122,'[Z Mode: '..auto.zMode[auto.zMode.index]..']')
    end
...
function extendedModeHud(hudText)
	...
	hudText:append(white..'Z Mode [Z]: '..white..auto.zMode[auto.zMode.index]..white)
	return hudText
end
