...
function init_gear_sets()
    ...
    sets.MagicFreeCast = set_combine(sets.MagicBurst, {
        ring1 = 'Shiva Ring +1',
        ring2 = "Shiva Ring +1"
    })
    sets.naked = {
    ...
    sets.precast.JA['Vallation'] = set_combine(sets.baseEnmity, {
        body = "Runeist Coat +1",
        legs = "Futhark Trousers +2"
    })
    sets.precast.JA['Valiance'] = set_combine(sets.baseEnmity, {
        body = "Runeist Coat +1",
        legs = "Futhark Trousers +2"
    })
    sets.precast.JA['Pflug'] = sets.baseEnmity
    sets.precast.JA['Battuta'] = set_combine(sets.baseEnmity, {
        head = "Futhark Bandeau +1"
    })
    sets.precast.JA['Liement'] = set_combine(sets.baseEnmity, {
        body = "Futhark Coat +1"
    })
    ...
    sets.precast.JA['Elemental Sforzo'] = set_combine(sets.baseEnmity, {
        body = "Futhark Coat +1"
    })
    sets.precast.JA['Swordplay'] = set_combine(sets.baseEnmity, {
        hands = "Futhark Mitons +1"
    })
    sets.precast.JA['Vivacious Pulse'] = set_combine(sets.baseEnmity, {
        head = "Erilaz Galea +2"
    })
    ...
    sets.precast.WS['Resolution'] = set_combine(sets.baseWS, {
        back = gear.STRCape,
        legs = "Samnuha Tights",
        ring1 = "Epona's Ring",
        ammo = "Seething Bomblet +1"
    })
    sets.precast.WS['Dimidiation'] = set_combine(sets.baseWS, {
        body = "Adhemar Jacket +1",
        back = gear.DEXCape,
        ammo = "Knobkierrie"
    })
    ...
    sets.precast.WS['Sanguine Blade'] = set_combine(sets.MagicFreeCast, {
    ...
    sets.midcast['Phalanx'] = set_combine(sets.baseTank, sets.interrupt, sets.emSkill, sets.emDuration, {
        head = "Futhark Bandeau +1",
        body = "Taeon Tabard",
        hands = "Taeon Gloves",
        feet = "Taeon Boots"
    ...
    sets.midcast['Regen'] = set_combine(sets.baseTank, sets.emDuration, {
        head = "Runeist Bandeau +1",
        right_ear = "Erilaz Earring",
        neck = "Sacro Gorget",
        waist = "Sroda Belt"
    })
    sets.midcast['Cure'] = set_combine(sets.baseTank, {
        waist = "Sroda Belt"
    })
    sets.midcast['Refresh'] = set_combine(sets.baseTank, sets.emDuration, {
        waist = "Gishdubar Sash",
        head = "Erilaz Galea +2"
    })
    sets.midcast['Cocoon'] = sets.baseTank
    sets.midcast['Stoneskin'] = set_combine(sets.baseTank, sets.emSkill, {
        waist = "Siegel Sash"
    })
...
function init_modetables()
    ...
    gearMode = {
        ...
        [0] = {
            name = "Tank",
            idle = (set_combine(sets.baseTank, sets.baseIdle)),
            engaged = (sets.baseTank)
        },
        [1] = {
            name = "DPS",
            idle = (set_combine(sets.baseMelee, sets.baseIdle)),
...
function extendedJobPrecast(spell, action, spellMap, eventArgs)
    ...
    if auto.buff[auto.buff.index] == 'On' and spell.english:contains('Protect') and
        not (windower.ffxi.get_ability_recasts()[72] > 0) then
        eventArgs.cancel = true
        send_command('input /ja "Embolden" <me>; wait 2; input /ma "' .. spell.name .. '" ' ..
...
function autoActions()
    ...
    if auto.buff[auto.buff.index] == 'On' and not actionInProgress and not moving then
        if auto.rune[auto.rune.index] == 'On' and buffCheck(currRune, 3) and runeRecast == 0 then
        ...
        -- Temper
        if not buffactive['Multi Strikes'] and
            (player.status == 'Engaged' or gearMode[gearMode.index].name ~= 'SuperTank') then
            -- actionInProgress = true
